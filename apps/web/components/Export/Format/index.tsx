...
const Format = ({ type }: FormatProps) => {
  ...
  const formats = type === 'video' ? formatsVideo : type === 'audio' ? formatsAudio : formatsImage
      ...
      {formats.map((format, index) => (
        ...
        >
          <div className="w-10 mr-4">
            <Image className="w-full" src={format.image} width={40} height={40} alt="" />
          </div>
          <div className="grow text-left">
            <div className="text-base-2 font-semibold">{format.title}</div>
            <div className="text-smaller text-n-4">{format.note}</div>
          </div>
        </button>
      ))}
    </div>
