    ...
    <>
      <div className="space-y-1">
        ...
        <Select
          ...
          onChange={setTone}
        />
        <Action title="English (United State)" icon="translate" onClick={() => console.log('English (United State)')} />
        <Action
          title="Fix spelling and grammar"
          icon="text-input"
          onClick={() => console.log('Fix spelling and grammar')}
        />
        <Action title="Improve speaking" icon="recording-1" onClick={() => console.log('Improve speaking')} />
      </div>
      <Export image="/images/video-pic.jpg" className="mt-4" type="video" details={detailsVideo} />
    </>
  )
}
