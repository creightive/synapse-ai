...
const Select = ({ label, iconHead, iconDropdown, className, items, value, onChange }: SelectProps) => (
        ...
        <>
          ...
          >
            <div className="flex justify-center items-center shrink-0 w-8 h-8 rounded-lg bg-n-2 transition-colors dark:bg-n-7">
              <Icon className="shrink-0 w-4 h-4 fill-n-5 dark:fill-n-3" name={iconHead} />
            </div>
            <div className="w-[calc(100%-3.25rem)] px-4 text-left">
              <div className="truncate text-smaller text-n-4">{label}</div>
              <div className="truncate text-base-2 font-semibold dark:text-n-3">{value.title}</div>
            </div>
            <Icon className={`fill-n-4 transition-transform ${open ? 'rotate-180' : ''}`} name="arrow-down" />
          </Listbox.Button>
          <Transition
            ...
            >
              {items.map((item: any) => (
                ...
                >
                  ...
                  <Icon className="hidden shrink-0 w-6 h-6 ml-4 mr-1 fill-color-3 ui-selected:block" name="check" />
                </Listbox.Option>
              ))}
            </Listbox.Options>
          </Transition>
