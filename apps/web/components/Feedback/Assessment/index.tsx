      ...
      {list.map((item, index) => (
        <div className="flex flex-col flex-1 md:flex-row" key={index}>
          <div className="flex justify-center items-center w-10 h-10 mb-6 rounded-xl shadow-2 md:shrink-0 md:mb-0 md:mr-4 dark:bg-n-1">
            <Image className="w-6" src="/images/annotation-check.svg" width={24} height={24} alt="" />
          </div>
          <div className="flex flex-col grow">
            <div className="mb-1 text-base-1 font-semibold md:text-base-2">{item.title}</div>
            <div className="mt-auto text-h2 md:text-h3">{item.percent}%</div>
          </div>
        </div>
      ))}
    </div>
    <div className="pb-3">
      <div className="table w-full md:block md:w-auto">
        <div className="table-row text-small text-n-4 md:flex">
          ...
          <div className="table-cell pl-5 py-2">Incorrect answer (5)</div>
          <div className="table-cell pl-5 py-2">Correct answer</div>
          <div className="table-cell pl-5 pr-10 py-2 text-center md:hidden">How</div>
        </div>
        {items.map(x => (
