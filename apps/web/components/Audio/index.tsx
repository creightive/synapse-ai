  ...
  return (
    <div className="relative rounded-xl bg-n-6 border border-n-5">
      ...
      <div className="flex items-center px-6 py-5">
        <button className="group shrink-0 w-10 h-10 mr-3 rounded-full bg-n-1 text-0" onClick={() => setPlay(!play)}>
          <Icon
            className={`w-4 h-4 fill-n-5 transition-colors group-hover:fill-primary-3 ${play ? 'w-4 h-4' : 'w-6 h-6'}`}
          ...
          <div className="border border-n-5 rounded-xl">
            <div className="p-3 text-n-1">
              <span className="text-primary-4">
                Synapse is an AI chat app UI Kit designed by the UI8 Team. It boasts a stunning design and powerful
                features, making it an ideal choice for developers looking to build chat applications with ease|
              </span>
              . With Synapse, you can quickly create chat interfaces that are both intuitive and visually appealing,
              thanks to its comprehensive set of pre-designed components and customizable elements. Whether you&apos;re
              building a messaging app, a customer support platform, or any other kind of chat-based application,
              Synapse has everything you need to get started.
            </div>
            <div className="flex justify-between items-center p-3 border-t border-n-5 md:block">
              <div className="text-small text-n-4">
                <p>
                  Word count: <span className="text-n-1">67</span>
                </p>
                <p>
                  Character count (including spaces): <span className="text-n-1">358</span>
                </p>
              </div>
              <button className="btn-light md:w-full md:mt-2" onClick={() => setSave(false)}>
                Save audio
              </button>
            </div>
          </div>
