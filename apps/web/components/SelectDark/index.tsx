      ...
      {({ open }) => (
        <>
          <Listbox.Button className="h-12 pl-6 pr-4 bg-n-6 border-r border-n-5 text-base-2 font-semibold text-n-1 tap-highlight-color">
            {value.title}
            <Icon
              className={`shrink-0 ml-3 fill-n-1 transition-transform ${open ? 'rotate-180' : ''}`}
              name="arrow-down"
            />
          </Listbox.Button>
            ...
            <Listbox.Options className="absolute t-full left-0 right-0 z-5 bg-n-6 shadow-modal">
              {items.map((item: any) => (
                ...
                >
                  ...
                  <Icon className="hidden shrink-0 w-6 h-6 ml-4 -mr-1 fill-n-1 ui-selected:block" name="check" />
                </Listbox.Option>
              ))}
            </Listbox.Options>
          </Transition>
