    ...
    <Transition show={visible} as={Fragment}>
      ...
      >
        <Transition.Child
          ...
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-n-7/90 dark:bg-n-7/95" aria-hidden="true" />
        </Transition.Child>
        <Transition.Child
          as={Fragment}
          enter="ease-out duration-300"
          ...
          <Dialog.Panel
            className={twMerge(
              `relative z-10 max-w-[48rem] w-full m-auto bg-n-1 rounded-[1.25rem] shadow-modal md:rounded-xl dark:bg-n-6 ${
                video ? 'max-w-[64rem] aspect-video bg-n-7 overflow-hidden' : ''
              } ${videoPlayer ? 'max-w-[64rem] bg-n-7 overflow-hidden' : ''} ${classWrap}`,
            )}
          >
            {children}
            <button
