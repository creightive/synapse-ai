...
const Chat = ({ newChat, background, generationMessage, typeChat, children }: ChatProps) => {
        ...
        >
          <div className="grow overflow-y-auto scroll-smooth scrollbar-none">
            <div className="relative h-64 max-h-950:h-48 max-h-850:h-40 bg-n-2 md:h-36 dark:bg-n-7">
              <Image className="object-cover" src={background} fill alt="" />
              <button
                ...
                onClick={() => setVisible(isTablet ? false : true)}
              >
                <Icon className="w-6 h-6 skew-x-[180deg] fill-n-1 text-0" name="arrow-square-left" />
              </button>
            </div>
          ...
          <Message
            ...
            // ]}
          />
          <RightSidebar typeChat={typeChat} visible={visible} onClick={() => setVisible(isTablet ? true : false)} />
        </div>
