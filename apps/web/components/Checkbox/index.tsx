...
const Checkbox = ({ className, label, value, onChange, reverse }: CheckboxProps) => (
  <label
  ...
  >
    ...
    >
      <Icon className={`fill-n-1 transition-opacity ${value ? 'opacity-100' : 'opacity-0'}`} name="check" />
    </span>
    {label && (
      <span className={`text-base-2 font-semibold text-n-5 dark:text-n-3 ${reverse ? 'mr-auto pr-3' : 'pl-3'}`}>
        {label}
      </span>
    )}
  </label>
