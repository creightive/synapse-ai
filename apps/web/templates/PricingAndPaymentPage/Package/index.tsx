...
const Package = ({ item, plan }: PackageProps) => {
  ...
  return (
    <>
      ...
      >
        <div className="relative flex flex-col grow z-2">
          <div className="flex items-center mb-1">
            ...
            {item.popular && <div className="ml-auto px-3 bg-color-3 rounded text-small text-n-1">Popular</div>}
          </div>
          <div className="mb-6 text-base-1">{item.description}</div>
          <div className="mb-2">
            <span className="relative -top-3.5 mr-1 text-h4 text-n-4">$</span>
            <span className="text-h2">{plan === 'monthly' ? item.priceMonth : item.priceYear}</span>
          ...
          <div className="grow mt-6 mb-8 pt-6 border-t border-n-3 dark:border-n-5">
            {item.details.map((x: any, index: number) => (
              <div className="flex mb-4 text-base-2 last:mb-0" key={index}>
                <Icon
                  className={`shrink-0 w-6 h-6 mr-3 ${x.disabled ? 'fill-n-4/50' : 'fill-n-4'}`}
                  name="check-circle"
                />
                {x.title}
              </div>
          ...
          <button
            className={`${item.popular ? 'btn-gradient' : 'btn-stroke'} ${
      ...
      <Checkout visible={visibleModal} onClose={() => setVisibleModal(false)} />
    </>
  )
}
