...
const Form = ({}: FormProps) => {
  ...
  const styleInput = 'w-full h-6 border-none bg-transparent text-base-1 outline-none placeholder:text-n-4'

  return (
    <form className="flex flex-col grow" action="" onSubmit={() => console.log('Submit')}>
      <div className="mb-3 p-5 rounded-2xl bg-primary-7">
        <div className="mb-1 text-h2 text-primary-3">Pro</div>
        <div className="text-body-1 dark:text-n-7">$20.00/month/1 team member</div>
      </div>
      <RadioGroup items={plans} value={plan} setValue={setPlan} />
      <div className="mb-3 border border-n-3 rounded-xl dark:border-n-5">
        <div className="p-5">
          <div className="mb-3 text-base-2 text-n-5 dark:text-n-3">Billing email</div>
          <div className="relative">
            <Icon className="absolute top-0 left-0 w-6 h-6 pointer-events-none fill-n-4" name="email" />
            <input
        ...
        <div className="p-5 border-t border-n-3 dark:border-n-5">
          <div className="mb-3 text-base-2 text-n-5 dark:text-n-3">Card details</div>
          <div className="flex md:flex-wrap">
            <div className="relative grow md:w-full md:mb-4">
              <Icon className="absolute top-0 left-0 w-6 h-6 pointer-events-none fill-n-4" name="credit-card" />
              <InputMask
                className={`${styleInput} pl-11`}
                {...cardNumber}
                type="tel"
