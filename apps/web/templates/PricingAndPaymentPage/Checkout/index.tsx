...
const Checkout = ({ visible, onClose }: CheckoutProps) => {
  return (
    <Transition show={visible} as={Fragment}>
      <Dialog className="fixed inset-0 z-50 flex p-4 overflow-auto scroll-smooth md:p-0" onClose={onClose}>
        <Transition.Child
          as={Fragment}
          ...
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-n-7/90" aria-hidden="true" />
        </Transition.Child>
        <Transition.Child
          as={Fragment}
        ...
        >
          <Dialog.Panel className="relative z-10 shrink-0 flex flex-col w-[37.25rem] min-h-full m-auto mr-0 -translate-x-[5%] bg-n-1 rounded-3xl md:w-full md:rounded-none md:min-h-screen-ios dark:bg-n-6">
            <div className="relative h-64 p-8 text-center md:h-56">
              <Image className="object-cover rounded-t-3xl md:rounded-none" src="/images/bg-checkout.jpg" fill alt="" />
              <div className="relative z-1w-[5.75rem] mx-auto">
                <Image src="/images/logo-vertical.svg" width={92} height={66} alt="" />
              </div>
              <button
