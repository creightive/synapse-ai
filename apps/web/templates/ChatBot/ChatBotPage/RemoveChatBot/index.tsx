...
const RemoveChatBot = ({}: RemoveChatBotProps) => {
  ...
  const handleClick = () => {
    ...
    toast(t => (
      <Notify className="md:flex-wrap md:rounded-2xl" iconDelete>
        <div className="ml-3 mr-16 text-base-1 font-semibold md:w-[calc(100%-2rem)] md:m-0 md:pl-3">Bot deleted</div>
        <Link
  ...
  return (
    <>
      ...
      >
        <Icon className="mr-2 fill-color-5 transition-colors lg:mr-0" name="trash-1" />
      ...
      >
        <div className="mb-2 text-h4 md:text-h5">Are you sure you want to delete this chatbot?</div>
        <div className="mb-10 text-body-1 text-n-4 md:mb-6">
          This action cannot be undone and all data will be permanently lost.
        </div>
        <div className="flex space-x-3">
          <button className="btn-red flex-1 md:px-2" onClick={handleClick}>
            <span>Yes, delete it</span>
            <Icon name="trash" />
          </button>
          <button className="btn-stroke flex-1 md:px-2" onClick={() => setVisibleModal(false)}>
            Cancel
          </button>
        </div>
      </Modal>
