...
const itemsEntry = [
  {
    content: 'The potential to enhance customer service and improve business efficiency',
  ...
  return (
    <div className="flex lg:block">
      ...
      <div className="relative flex flex-col grow p-8 lg:min-h-screen lg:min-h-screen-ios md:px-5">
        <button className="btn-stroke relative z-2 ml-auto -mb-12" onClick={() => setCreate(!create)}>
          <span>{create ? 'Sign in' : 'New account'}</span>
          <Icon name="entry" />
        </button>
        <div className="max-w-[25rem] m-auto py-14 md:pt-20 md:pb-0">
          <div className="mb-8 text-center text-h3">{create ? 'Welcome to Synapse' : 'Sign in'}</div>
          <div className="">
            ...
            <button className="btn border-2 border-n-3 w-full rounded-xl hover:bg-n-3 dark:border-n-5 dark:hover:bg-n-5">
              <Image
                src={isDarkMode ? '/images/apple-light.svg' : '/images/apple-dark.svg'}
                width={24}
                height={24}
                alt=""
              />
          ...
          <div className="flex items-center my-5">
            ...
            <span className="shrink-0 mx-6 text-small text-n-4">Or</span>
            <span className="grow h-0.25 bg-n-3 dark:bg-n-5"></span>
          </div>
