...
const Search = ({ className, value, onChange, onSubmit }: SearchProps) => (
  <form className={twMerge(`relative ${className || ''}`)} action="" onSubmit={onSubmit}>
    <button className="group absolute top-3 left-4 text-0" type="submit">
      <Icon className="absolute z-1 w-6 h-6 fill-n-4 transition-colors group-hover:fill-primary-4" name="search" />
    </button>
    <input
