...
const Select = ({
  ...
  <div className={`relative z-1 ${className || ''}`}>
    {label && <div className="flex mb-2 text-base-2 font-semibold text-n-4">{label}</div>}
          ...
          >
            {value?.image && (
              <div className="shrink-0 mr-3">
                <Image className="w-6" src={value.image} width={24} height={24} alt="" />
              </div>
            )}
            {value?.icon && <Icon className="mr-3 dark:fill-n-1" name={value.icon} />}
            <div className="grow mr-auto text-left truncate">{value.title}</div>
            <Icon
            ...
            <Listbox.Options
              className={twMerge(
                `absolute t-full left-0 right-0 z-5 mt-2 p-2 bg-n-1 rounded-2xl shadow-modal dark:bg-n-7 ${
                  open ? 'z-10' : ''
                } ${up ? 'top-auto bottom-full mt-0 mb-2' : ''} ${classOptions}`,
              )}
            >
              {items.map((item: any) => (
                <Listbox.Option
                ...
                >
                  {item.image && (
                    <div className="shrink-0 mr-3">
                      <Image className="w-6" src={item.image} width={24} height={24} alt="" />
                    </div>
                  )}
                  {item.icon && (
                    <Icon
