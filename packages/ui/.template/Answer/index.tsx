...
const Answer = ({ content, message, loading, children, result, hideActions }: AnswerProps) => {
    ...
    <div className="p-8 border-t border-n-3 md:px-5 dark:border-n-5">
      ...
      <div className="flex">
        <div className="relative shrink-0 w-8 h-8 p-1.5 bg-n-7 rounded-full">
          <Image className="w-full opacity-100" src="/images/logo-1.svg" width={20} height={20} alt="Avatar" />
          {shareVisible && <ShareChatButton />}
        </div>
        <div className="w-[calc(100%-2rem)] pl-6 space-y-6 md:pl-4">
          {content && <div className="pt-1 text-body-2 text-n-6 dark:text-n-1">{content}</div>}
          {children}
          {!hideActions && <Actions />}
        </div>
      </div>
