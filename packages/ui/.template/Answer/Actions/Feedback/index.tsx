  ...
  return (
    <div className="mt-6 pt-6 border-t border-n-3 dark:border-n-5">
      <div className="flex justify-between items-center mb-5">
        ...
        <button className="group shrink-0 w-10 h-10 ml-6 text-0" onClick={onClose}>
          <Icon className="transition-colors group-hover:fill-color-5 dark:fill-n-3" name="close" />
        </button>
      </div>
        ...
        {feedbackVariants.map(item => (
          <button
            className={`shrink-0 btn-stroke mx-1.5 ${
              activeId === item.id ? '!border-n-7 !text-n-7 dark:!border-n-2 dark:!text-n-2' : ''
            }`}
            key={item.id}
