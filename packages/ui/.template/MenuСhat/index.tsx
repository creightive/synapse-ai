      ...
      <Menu>
        {buttonIcon ? (
          <Menu.Button className="flex justify-center items-center w-11 h-11">
            <Icon className="w-6 h-6 fill-n-5 transition-colors ui-open:fill-primary-3 dark:fill-n-3" name="message" />
          </Menu.Button>
        ) : (
          <Menu.Button className="flex items-center max-w-full text-left text-h6 transition-colors md:text-body-1 hover:text-primary-3 tap-highlight-color">
            {historyChat
              ...
              .map(item => (
                <div className="relative pl-15 py-2 truncate md:pl-0" key={item.id}>
                  <div
            ...
            <Icon className="shrink-0 ml-4 fill-n-5 dark:fill-n-3" name="arrows-sorting" />
          </Menu.Button>
        )}
        <Transition
              ...
              {historyChat.map((item: any) => (
                <Menu.Item
                  className={`flex items-center p-3 rounded-xl text-base-1 font-semibold transition-colors hover:bg-n-2 dark:hover:bg-n-6/70 tap-highlight-color ${
                    item.url === pathname ? '!bg-n-7 !text-n-1 dark:!bg-n-6' : ''
                  }`}
                  key={item.id}
                  as="a"
                  href={item.url}
                >
                  <div
                    ...
                    style={{ backgroundColor: item.color }}
                  >
                    <Icon className="w-4 h-4 fill-n-1" name="chart-breakout-square" />
                  </div>
                  <div className="w-[calc(100%-3.25rem)] md:w-[calc(100%-2.5rem)] truncate">{item.title}</div>
                </Menu.Item>
              ))}
            </div>
          </Menu.Items>
