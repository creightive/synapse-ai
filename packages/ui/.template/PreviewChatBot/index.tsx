...
const PreviewChatBot = ({}: PreviewChatBotProps) => {
  ...
  return (
    <>
      <button className="btn-stroke md:h-10" onClick={() => setVisibleModal(true)}>
        Preview
      </button>
      <Modal
      ...
      >
        ...
        <div className="relative -mt-10 mb-16 px-10 md:px-5 md:mb-10">
          <div className="flex justify-center items-center shrink-0 w-20 h-20 mb-6 bg-n-7 rounded-[1.25rem]">
            <Image className="w-5" src="/images/logo-2.svg" width={20} height={20} alt="" />
          </div>
          ...
          <div className="flex">
            <div className="relative shrink-0 w-8 h-8 p-1.5 bg-n-7 rounded-full">
              <Image className="w-full" src="/images/logo-1.svg" width={20} height={20} alt="Avatar" />
            </div>
            <div className="w-[calc(100%-2rem)] pl-6">
              <div className="mb-6 text-n-6 dark:text-n-1">
                Hello, I’m Synapse a large language model designed to answer questions and provide information on a wide
                range of topics. How can I assist you today?
              </div>
              <div className="mb-6 text-n-6 font-semibold dark:text-n-1">Here is a few of what I can do for you</div>
        ...
        <Message value={message} onChange={(e: any) => setMessage(e.target.value)} hideButtonAdd />
      </Modal>
    </>
  )
}
