...
const comments = [
  ...
  },
  {
    ...
    content: 'Well-organized and coherent, but needs to improve grammar, vocabulary, and sentence structure.',
  },
  {
  ...
  <div className="">
    ...
    <div className="flex flex-wrap -mt-6 -mx-3">
      {comments.map((comment, index) => (
        <div className="w-[calc(50%-1.5rem)] mt-6 mx-3 lg:w-[calc(100%-1.5rem)]" key={index}>
          <div className="mb-2 text-h6">{comment.title}</div>
          <div className="text-n-6 dark:text-n-1">{comment.content}</div>
        </div>
      ))}
    </div>
  </div>
