    ...
    <>
      <div className="relative rounded-xl bg-n-6 border border-n-5 dark:bg-n-7">
        <div className="flex justify-between bg-n-7 rounded-t-xl shadow-[inset_0_-0.0625rem_0_0_#393E44]">
          {items ? (
            <>
              <SelectDark items={items} value={value} onChange={setValue} />
          ...
          {items ? (
            items.filter((x: any) => x === value).map((item: any) => <ButtonCopy key={item.id} text={item.value} />)
          ) : (
        ...
        <div className="max-h-[26.5rem] overflow-auto rounded-b-xl">
          {items ? (
            items
              ...
              .map((item: any) => <CodeSyntax language={item.language} key={item.id} value={item.value} />)
          ) : (
            <CodeSyntax language={code.language} value={code.value} />
          )}
        </div>
      </div>
      {content && <div className="mt-6 text-n-6 dark:text-n-3">{content}</div>}
